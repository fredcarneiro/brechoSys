...
<div class="row" style="padding-top: 1.563em;">
    <div class="twelve large-12 columns" style="padding-bottom:2em;  padding-left:0px; text-align:center; font-weight:bold;">
        RELAT&Oacute;RIO DE PRODUTOS CADASTRADOS
    </div>
</div>
<hr />
<div class="row" style="padding-top: 1.563em;">
    <div class="twelve large-12 columns">
        <table>
			...
			foreach($dbh->query($sql) as $row) {
				...
				$dt_cadastro = substr($row['dt_cadastro'], -2).'/'.substr($row['dt_cadastro'], 4, 2).'/'.substr($row['dt_cadastro'], 0, -4);
				
				
				
				if($row['dt_baixa']){
					$dt_baixa = substr($row['dt_baixa'], -2).'/'.substr($row['dt_baixa'], 4, 2).'/'.substr($row['dt_baixa'], 0, -4);
				}else{
					$dt_baixa = "N/D";
				}
				
				if($row['dt_repasse']){
					$dt_repasse = substr($row['dt_repasse'], -2).'/'.substr($row['dt_repasse'], 4, 2).'/'.substr($row['dt_repasse'], 0, -4);
				}else{
					$dt_repasse = "N/D";
				}				
